...
function Invoke-DomainConfigMenu {
    ...
    do {
        $ChoiceMenu = Show-Menu -Title "Basic Config" -MenuOptions @(
            ...
            "7: Create shares",
            "8: Set Permissions for directories and shares (automatic via csv file)"
            "",
            "9: Interactive shell"
        )
    ...
    try {
        ...
        foreach ($Group in $Groups) {
            ...
            if (-not (Get-ADGroup -filter "Name -eq '$GroupName'" -ErrorAction SilentlyContinue)) {
                ...
                Write-Host "    Group created: $GroupName ($Scope) in $OUName" -ForegroundColor Green
...
function Add-DomainUsers {
    ...
    try {
        ...
        $HomeDir    = $UserSettings.homeFolder.location
        $ProfileDir = $UserSettings.profileFolder.location

        Foreach ($Dir in @($HomeDir, $ProfileDir)) {
            if (-not(Test-Path -Path $Dir)) {            
                New-Item -Path $Dir -ItemType Directory -Force | Out-Null
                Write-Log -Message "Add-DomainUsers: Created $dir"
            }
        }
...
function Add-UsersToSecurityGroups {
    <#
    .SYNOPSIS
        Voegt domain users toe aan global security groups en nestelt die in domain local groups (AGDLP).

    .DESCRIPTION
        Leest users.json en verwerkt de securityGroups per user volgens het AGDLP-principe:
            - GL_ groepen: de user wordt er rechtstreeks aan toegevoegd.
            - DL_ groepen: de GL_ groepen van dezelfde user worden als lid toegevoegd aan de DL_ groep.
        Niet-bestaande groepen worden overgeslagen met een melding in de log.
        Alle acties worden weggeschreven naar de logfile.
    ...
    try {
        ...
        foreach ($User in $Users) {
            ...
            $GlobalGroups = $User.securityGroups | Where-Object { $_ -like "GL_*" }
            $LocalGroups  = $User.securityGroups | Where-Object { $_ -like "DL_*" }

            #User toevoegen aan zijn GL_ groepen
            foreach ($GroupName in $GlobalGroups) {
                if (Get-ADGroup -Filter "Name -eq '$GroupName'" -ErrorAction SilentlyContinue) {
                    try {
                        Add-ADGroupMember -Identity $GroupName -Members $Login -ErrorAction Stop
                        Write-Host "    Added '$Login' to global group: $GroupName" -ForegroundColor Green
                        Write-Log -Message "Added user '$Login' to global group '$GroupName'."
                    }
                    catch {
                        Write-Host "    [!] Failed to add '$Login' to '$GroupName': $($_.Exception.Message)" -ForegroundColor Red
                        Write-Log -Message "ERROR in Add-UsersToSecurityGroups: failed to add '$Login' to '$GroupName': $($_.Exception.Message)"
                    }
                }
                else {
                    Write-Host "    [!] Global group not found, skipping: $GroupName" -ForegroundColor Red
                    Write-Log -Message "ERROR in Add-UsersToSecurityGroups: global group '$GroupName' not found, skipped for '$Login'."
                }
            }

            #GL_ groepen van deze user nestelen in de DL_ groepen (AGDLP)
            foreach ($DLName in $LocalGroups) {
                if (-not (Get-ADGroup -Filter "Name -eq '$DLName'" -ErrorAction SilentlyContinue)) {
                    Write-Host "    [!] Domain local group not found, skipping: $DLName" -ForegroundColor Red
                    Write-Log -Message "ERROR in Add-UsersToSecurityGroups: domain local group '$DLName' not found, skipped."
                    continue
                }

                foreach ($GLName in $GlobalGroups) {
                    if (-not (Get-ADGroup -Filter "Name -eq '$GLName'" -ErrorAction SilentlyContinue)) {
                        continue  # al gemeld in de GL-loop hierboven
                    }

                    try {
                        Add-ADGroupMember -Identity $DLName -Members $GLName -ErrorAction Stop
                        Write-Host "    Added '$GLName' into domain local group: $DLName" -ForegroundColor Green
                        Write-Log -Message "Added global group '$GLName' into domain local group '$DLName'."
                    }
                    catch {
                        Write-Host "    [!] Failed to add '$GLName' into '$DLName': $($_.Exception.Message)" -ForegroundColor Red
                        Write-Log -Message "ERROR in Add-UsersToSecurityGroups: failed to add '$GLName' into '$DLName': $($_.Exception.Message)"
                    }
                }
            }
        }
        ...
        Write-Host "    ERROR: The Users.json file was not found. Please recreate it." -ForegroundColor Red
...
function Join-Domain {
    ...
    try {
        ...
        Read-Host -Prompt "`n     Enter to Reboot"    
